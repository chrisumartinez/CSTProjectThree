...
class canvasPainting(Widget):
    canvasPaintingButton = Button()
    def callback(self):
	    ...
	    cv2.putText(img, "K - Switch Drawing&Trail modes", (50,340), cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)
            cv2.putText(img, "A - Draw", (50,360), cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)
            cv2.putText(img, "S - Erase the drawing", (50,380), cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)
            cv2.putText(img, "I - Change Size/ Color", (50,400), cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)
            cv2.putText(img, "J - Decrease Size/Color", (50,420), cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)
            cv2.putText(img, "L - Increase Size/Color", (50,440), cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)
            #////////////////////////
        ...
        return
    canvasPaintingButton.bind(on_release = callback)
def upload(self):
...
class uploadButton(Widget):
    ...
    def callback(self):
        ...
        print(uploaded_image.link)  #Prints the link to the image on imgur
        return
    uploadButton.bind(on_release = callback)
class GUI(Widget):
    pass
class GUIApp(App):
    def build(self):
        newGUI = GUI()
        newButton = uploadButton()
        canvasButton = canvasPainting()
        newGUI.add_widget(newButton)
        newGUI.add_widget(canvasButton)
        return newGUI
